defmodule Bonfire.UI.Social.CommentsEmbedCreationGateTest do
  @moduledoc """
  The generic `media_uri` branch creates a thread anchor on demand, unauthenticated, attributed
  to the instance's configured import author — which previously let anyone (a crawler, a curl,
  a forged iframe URL) mint public posts under that identity for arbitrary URIs.

  Creation (unlike reading) is therefore gated (see `EmbedCommentsLive.maybe_create_anchor/3`):
  a guest-loaded embed may only anchor URIs on origins allowlisted in `IFRAME_ALLOWED_ORIGINS`,
  while a signed-in viewer may anchor any URI since it's attributed to them. Reading an existing
  thread needs neither the allowlist nor a configured creator.
  """
  ...
  test "an allowlisted host does not authorise other origins on the same host scheme-swapped" do
    allowlist!("https://blog.example.com")
    uri = "http://blog.example.com/plaintext-origin/"
  ...
  test "a signed-in viewer may anchor a URI on any origin (attributed to them)" do
    ...
    assert {:ok, media} = Media.get_by_path(uri)
    assert media.creator_id == user.id
  end
