defmodule Bonfire.Social.Objects.LiveHandler do
  ...
  def load_object_assigns(%{object_id: id, object: %{id: pre_loaded}, activity: _} = assigns, socket) when is_binary(id) and id==pre_loaded do
    debug(pre_loaded, "object pre_loaded")
    init_object_assigns(assigns.object, assigns.activity, assigns, socket, l("Discussion"))
  end
  def load_object_assigns(%{post_id: id, object: %{id: pre_loaded}, activity: _} = assigns, socket) when is_binary(id) and id==pre_loaded do
