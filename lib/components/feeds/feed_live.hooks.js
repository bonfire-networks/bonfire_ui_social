let FeedScrollTracker = {
  mounted() {
    ...
    let enabled = this.el.hasAttribute("data-save-position");

    console.log("[FeedScrollTracker] mounted, feedName=" + this.feedName + " enabled=" + enabled);
    ...
    if (!enabled || !this.feedName) return;
  ...
  savePosition() {
    ...
    if (id && id != this.previousID) {
      ...
      window.Bonfire.setBonfireParam("reading_pos", this.feedName, id, 172800000);
