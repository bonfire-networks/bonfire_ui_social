export default {
  ...
  async calculateOptimalRatio() {
    ...
    // A single slide has no siblings to share a height with, so use the media's
    // exact ratio (clamped to sane bounds) instead of snapping to a bucket —
    // avoids letterboxing/cropping from a near-miss bucket.
    const optimalRatio = this.items.length === 1
      ? Math.min(Math.max(minRatio, this.RATIOS.PORTRAIT_TALL), this.RATIOS.LANDSCAPE_WIDE)
      : this.snapToRatioBucket(minRatio);
    this.applyRatio(optimalRatio);
  },

  getMediaDimensions(el) {
    // Prefer server-provided dimensions (from stored/backfilled media metadata) so the carousel
    // can compute its aspect-ratio on mount — before images finish loading — avoiding layout shift.
    const dw = parseInt(el.dataset.width, 10);
    const dh = parseInt(el.dataset.height, 10);
    if (dw > 0 && dh > 0) {
      return Promise.resolve({ width: dw, height: dh });
    }
